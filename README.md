...
在本次报告中，我们使用了**K-均值聚类，K-中间点聚类，系谱聚类，密度聚类， 期望最大化聚类**等经典聚类方法，在多个不同的UCI数据集上进行聚类测试，并试图评估方法之间的优缺点以及其在运用中可能会碰到的实际问题。
...
在报告的第二部分中，与第一份报告不同，我们将按照**数据集的顺序**，依次介绍每个数据集的预处理、聚类操作以及探索性分析。并结合每一步的结果提出思考。
